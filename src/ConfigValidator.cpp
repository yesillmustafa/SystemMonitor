...
std::vector<std::string> ConfigValidator::Validate(const Config& config)
{
    ...
    {
        errors.push_back(ConfigError::ValidationError("CPU", "intervalSeconds must be between 1 and 60"));
    }

    if (config.m_cpu.warningThreshold <= 0 || config.m_cpu.warningThreshold > 100)
    {
        errors.push_back(ConfigError::ValidationError("CPU", "warningThreshold must be between 0-100"));
    }

    if (config.m_cpu.criticalThreshold <= 0 || config.m_cpu.criticalThreshold > 100)
    {
        errors.push_back(ConfigError::ValidationError("CPU", "criticalThreshold must be between 0-100"));
    }

    if (config.m_cpu.warningThreshold >= config.m_cpu.criticalThreshold)
    {
        errors.push_back(ConfigError::ValidationError("CPU", "warningThreshold must be less than critical"));
    }

    // RAM
    if (config.m_ram.intervalSeconds <= 0 || config.m_ram.intervalSeconds > Defaults::MAX_INTERVAL)
    {
        errors.push_back(ConfigError::ValidationError("RAM", "intervalSeconds must be between 1 and 60"));
    }

    if (config.m_ram.warningThreshold <= 0 || config.m_ram.warningThreshold > 100)
    {
        errors.push_back(ConfigError::ValidationError("RAM", "warningThreshold must be between 0-100"));
    }

    if (config.m_ram.criticalThreshold <= 0 || config.m_ram.criticalThreshold > 100)
    {
        errors.push_back(ConfigError::ValidationError("RAM", "criticalThreshold must be between 0-100"));
    }

    if (config.m_ram.warningThreshold >= config.m_ram.criticalThreshold)
    {
        errors.push_back(ConfigError::ValidationError("RAM", "warningThreshold must be less than critical"));
    }

    // Process
    if (config.m_process.intervalSeconds <= 0 || config.m_process.intervalSeconds > Defaults::MAX_INTERVAL)
    {
        errors.push_back(ConfigError::ValidationError("PROCESSMONITOR", "intervalSeconds must be between 1 and 60"));
    }

    // Application
    if (config.m_app.sleepMs <= 0 || config.m_app.sleepMs > Defaults::MAX_SLEEP)
    {
        errors.push_back(ConfigError::ValidationError("APPLICATION", "sleepMs must be between 0-10000 ms"));
    }

    // Logger
    if (config.m_logger.enableFileLog && config.m_logger.filePath.empty())
    {
        errors.push_back(ConfigError::ValidationError("LOGGER", "File logging enabled but filePath is empty"));
    }
